...
Scroller = function( settings ) {
	...
	this.throttle         = false;
	this.handle           = '<nav role="navigation" id="nav-below" class="site-navigation paging-navigation jpkis-nav-only"><div class="'+jpkis_opt.button_class+ ' zoom-btn jpkis-only" id="infinite-handle"><span><button>' + text.replace( '\\', '' ) + '</button></span></div></nav>';
	...
	if ( type == 'scroll' ) {
		...
		if ( this.click_handle ) {
			$('.infinite-scroll .infinite-scroll-area').append( this.handle );
		}

		this.body.delegate( '#infinite-handle', 'click.infinity', function() {
			// Handle the handle
			if ( self.click_handle ) {
				$( '.jpkis-nav-only' ).remove();
			}
...
Scroller.prototype.refresh = function() {
	...
	if ( this.click_handle ) {
		loader = '<nav role="navigation" id="nav-below" class="site-navigation paging-navigation jpkis-scroll-only"><div class="bubblingG"><span id="bubblingG_1"></span><span id="bubblingG_2"></span><span id="bubblingG_3"></span></div></nav>';
		$('.infinite-scroll .infinite-scroll-area').append( loader );

		loader = $('.infinite-scroll .infinite-scroll-area').find( '.jpkis-scroll-only' );
			...
			if ( response.type == 'success' ) {
				...
				if ( type == 'click' ) {
					if ( response.lastbatch ) {
						...
						if ( self.click_handle ) {
							$('.infinite-scroll .infinite-scroll-area').append( self.handle );
						} else {
